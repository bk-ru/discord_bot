        ...
        if is_admin or any("преподаватель" in r for r in roles):
            embed.add_field(
                ...
                value=(
                    ...
                    "`!accept @студент <номер>` — Зачесть лабораторную работу.\n"
                    "`!labfile @студент <номер>` — Получить ссылку на файл отправленной работы.\n"
                    "`!resubmitlab @студент <номер>` — Заменить файл и переотправить работу в канал преподавателя.\n"
                    "`!deletelab @студент <номер>` — Удалить работу и связанные с ней сообщения."
                ),
                inline=False
            )
